...
PH_output FTM_HitProcess :: ProcessHit(MHit* aHit, gemc_opts Opt)
{
 string hd_msg   = Opt.args["LOG_MSG"].args + " FTM Hit Process " ;
 double VERB     = Opt.args["HIT_VERBOSITY"].arg;

 PH_output out;
 out.identity = aHit->GetId();

 HCname = "FTM Hit Process";
...
vector<identifier>  FTM_HitProcess :: ProcessID(vector<identifier> id, G4Step* aStep, detector Detector, gemc_opts Opt)
{
 double x, y, z;
